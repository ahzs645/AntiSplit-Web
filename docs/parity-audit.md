...
| Corpus confidence | Tooling implemented; the currently available iHunter split container passes in `v1` and `v1-v2` modes. | `npm run verify:corpus -- --report docs/corpus-results.md`, `npm run verify:corpus -- --manifest docs/local-corpus-manifest.txt --report docs/corpus-manifest-results.md` |
...
| `resources.arsc` merge | Implemented for current single-package corpus shapes, including styled string-span preservation/remapping, split string remaps, type-spec merging, standard/compact/sparse type chunks, and complex map entries. | `npm run verify:fixture`, `npm run verify:corpus -- --report docs/corpus-results.md` |
| ARSC shape scan | Implemented; iHunter exercises styled string pools in both the base and a language split. | `npm run scan:arsc -- --manifest docs/local-corpus-manifest.txt --report docs/arsc-shape-report.md`, `docs/arsc-shape-report.md` |
| ARSC unsupported guards | Implemented fail-closed checks for malformed string pools, multi-package tables, and missing base type IDs. | `npm run verify:arsc-guards` |
...
- The iHunter sample passes.
- The corpus reports record each input's byte size and SHA-256 so fixture identity is reproducible.
- It passes `JAR/v1` and `JAR/v1 + v2`.
- It verifies with bundled Java apksig: `ok v1` for the legacy Android range and `ok v2` for Android 7.0+; the v2 output also verifies with Android SDK `apksigner`.
...
- The iHunter input and output ARSC shape scans record styled pools with no unsupported shapes, and Android SDK `aapt2 dump resources` accepts the merged output without warnings.
...
| Styled string pool span preservation | Implemented and verified with iHunter; output is accepted without warnings by Android SDK `aapt2 dump resources`. | Keep iHunter in the fixture corpus as regression coverage. |
